...
import { client } from "@/lib/appwrite-config"; // Ton client Appwrite
...
// Fonction utilitaire pour générer l'URL du logo
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        ...
        >
          <div
            className={`flex aspect-square size-8 items-center justify-center overflow-hidden rounded-lg bg-gradient-to-br ${getPlanColor(
              selectedOrg.plan,
            )} text-white`}
          >
            {selectedOrg.logo ? (
              <img
                src={getLogoUrl(selectedOrg.logo)}
                alt="Logo"
      ...
      >
        ...
        {organizations.map((org) => (
          ...
          >
            <div
              className={`flex size-6 items-center justify-center overflow-hidden rounded-sm bg-gradient-to-br ${getPlanColor(
                org.plan,
              )}`}
            >
              {org.logo ? (
